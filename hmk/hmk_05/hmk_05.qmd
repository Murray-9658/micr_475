...
Using `!(is.na())` we can filter out values that are `NA`. `!` Is used to say **not**, and `is.na()` determines if NAs are present.

By filtering for **not NA** values of arrival and departure times, we can omit them from our data set.

The OR operator, `|` is used to omit the NAs within the arrival OR departure columns.
